...
> **权威产品态见 [`PRODUCT_VISION_AND_ROADMAP.md`](PRODUCT_VISION_AND_ROADMAP.md)——请先读它。**
> 本文是**底座驱动机制**(持续会话 + 流式事件)的设计叙事,这一层**仍然现行且准确**:
> 三个底座(claude-code / codex / opencode)统一为一个**持续会话 + 流式事件**抽象,
> 持续会话是默认、单发是 fail-open 兜底,这些都没变。
>
> 但本文写于**智能路由 / 可视计划 / 团队调度 / 固件注入 / repo-map**落地之前,
> 行文仍以"9 阶段流水线"为主线叙述。请按这样理解二者关系:持续会话是**底层的会话机制**;
> 而 VISION 描述的 router→plan→schedule→deliver(总监如何路由一条输入、拆出可视计划、
> 逐步调度团队)是**叠加在这个会话之上的运行时**。文中"9 阶段"应读作"总监为完整商业级
> 交付选择的最深一招",而非每条输入的固定漏斗(见 README"流水线设计"与 spec §4.1 / §9.5)。
> 这套"持续会话取代每阶段单发"的设计原则本身,VISION 完全继承。
