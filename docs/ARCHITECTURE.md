...
> 把它们拧成一个交付系统;一个**协调者席位**负责调度与把关。治理(32 条 clause)是它的
> 地板安全网。

## 一句话定位

UmaDev 是**一个模拟真实开发团队来工作的 Coding Agent**——产品经理、架构师、UI/UX 设计师、前端、后端、QA、安全、
DevOps 八个角色像真实团队一样在共享黑板上各做各的专业活;一个**协调者席位**(技术负责人)负责调度与把关。它加载你已登录的 AI 编码 CLI(一等支持恰好三个底座:Claude Code /
Codex / OpenCode)的大脑当团队的脑子,协调者智能路由你的需求、拆出可视计划、注入团队心法 +
对你代码库的理解、逐步调度这八个角色并每步验收、留下交付证明;"AI 不能写什么"的治理
(32 条 clause)是底层的安全网。独立开发者或小团队,因此瞬间拥有一支完整、有工程纪律的开发团队。
