...
export const releases = {
  zh: [
    { ver: "1.0.13", date: "2026-06-28", current: true, title: "渲染健壮性:抄 Claude Code 的 DEC 2026 同步输出修界面错乱 + 防御泄漏的鼠标序列 + Ctrl+L 强制重绘", changes: [["修复", "界面跑久了显示错乱 / 刷新撕裂(Windows 尤其明显):抄 Claude Code 的做法,用 DEC 2026 同步输出把整帧原子刷新 —— 终端缓冲整帧再一次性交换,刷新中途不再出现半成品的错乱画面;按终端类型检测支持(iTerm2 / WezTerm / ghostty / kitty / Windows Terminal 等,跳过 tmux)"], ["修复", "滚动时输入框出现乱码 + 假的「本轮已中止」:鼠标滚动的 SGR 序列在高负载下偶发被劈碎,ESC 被当退出键、其余进了输入框。现在加了防御式过滤,识别并丢弃被劈碎的鼠标序列,真正的 Esc 和正常输入不受影响"], ["新增", "Ctrl+L / /redraw 强制全屏重绘 —— 万一画面还是花了一键恢复;窗口 resize 时清屏(某些终端 resize 后留残影);长路径 / Tab 裁切到可视宽度,不再溢出串行"]] },
