...
export const releases = {
  zh: [
        { ver: "1.0.34", date: "2026-07-06", current: true, title: "init 像 Claude Code /init 一样理解项目 - spec 阶段误判修复", changes: [["修复", "spec 阶段不再误报 build claimed done but no source:底座在 docs/spec 阶段写 PRD/架构/UIUX/SRS 文档时,以前任何工作区写入都被当成写代码构建、触发源码 QC、没代码就误报每回合重复。现在写文档(output/、.md、.umadev/)不再翻成代码构建;只有真写代码才触发,后续真写代码仍会触发,空/未知路径当代码(绝不放过真幻觉),诚实地板本身不动"], ["新增", "umadev init 现在像 Claude Code 的 /init 一样理解项目:重执行时检测技术栈 + build/test/lint 命令 + 索引源码,把一个受管的 Project 段写进 CLAUDE.md;重跑只刷新该段,治理前言和你的手改都保留。此前只写死模板、重跑什么都不更新,现在真正分析项目 + 刷新详细索引/进度"]] },
        { ver: "1.0.33", date: "2026-07-06", current: false, title: "第三方底座零配置可用 - 底座并发闸根治 GLM 529 限流", changes: [["修复", "第三方低并发底座(如 GLM / open.bigmodel.cn)不再撞 529 访问量过大:UmaDev 是一支团队,常态下可能同时握着多个底座进程(常驻聊天会话 + 后台预热会话 + 构建时每个 critic 各自 fork 的会话),而直连底座永远只有 1 条连接;这些额外并发连接超过第三方网关的小并发配额,就被 529 全拒、每回合都炸。新增全局底座调用并发闸(base_gate):每次真正打底座的回合都先拿一个许可,默认并发=1,和直连底座完全一样的足迹 -- 接入即用,任何底座、零配置"], ["改进", "强底座提速开关:官方高并发端点的用户可设 UMADEV_BASE_CONCURRENCY=4 恢复 critic 并行 + 预热加速,是给进阶用户的隐形开关,普通用户永远不用碰"], ["改进", "并发闸工程细节:许可为单次回合粒度、结构上不可能自锁;预热用 try_acquire 拿不到就跳过、绝不阻塞你的回合;聊天 firmware 本就是 Light 档(仅身份),单次调用不因此变重"]] },
        { ver: "1.0.32", date: "2026-07-06", current: false, title: "研究驱动的开发团队交互强化 · typed 交接契约全线打通 · 门禁显示一致性", changes: [["新增", "座位能力卡(Seat Card):每个座位(PM/架构/设计/前端/后端/QA/安全/DevOps)有一张 typed 自描述能力卡,声明它 owns 什么、reads 哪些契约输入、produces 哪些产出;roster 自描述,每次交接都成为显式可校验的契约"], ["新增", "每跳交接校验(双向):座位运行前即比对声明的输入/产出与黑板现状 —— 缺声明输入(错位)或没产出其负责的 artifact(规格缺口)都在交接处折进 verdict 暴露,不再变成下游谜团,活在评审流里"], ["新增", "两层 artifact 物化:把散文文档里的决策抽成 typed 契约(数据模型/设计令牌/验收标准)emit 到 .umadev/contracts/,与 API 契约并列,接进文档阶段"], ["新增", "verdict 结构化溯源:评审结论可带来源座位 + 关联 artifact + 诊断说明,由确定性标注器填充(不采信底座自述),打回更可诊断、审计链可重建;serde 向后兼容"], ["新增", "artifact 版本化 → DAG 陈旧失效:黑板文档带内容版本;跨会话改了 PRD/架构/UIUX 后 resume,消费该文档的步骤及下游自动翻回待办、对着变更后的上游重新推导,不信任被静默污染的旧结果"], ["新增", "黑板 public/private 私有区:两座位化解冲突时可在 .umadev/scratch/ 私有区来回,不污染公共 output/ 黑板,run 结束回收"], ["修复", "守护模式门禁文案一致:预检卡门禁行随信任模式(auto/guarded/plan)动态生成,不再底部写「逐门审核」正文却写「默认自动通过」;去掉意图卡重复的「完整构建」行"]] },
...
{ ver: "1.0.14", date: "2026-06-28", current: false, title: "Ctrl+C 不再误退 · /continue 跨会话续接", changes: [["修复", "Ctrl+C 不再退出 UmaDev:它是复制的通用肌肉记忆,误按一下就退出太容易丢会话。现在 Ctrl+C 只清空半截输入 / 提示用 /quit,绝不退出;要退出用 /quit、/exit、Ctrl+D 或双击 Esc"], ["新增", "/continue 跨会话续接:之前 /run 跑过、关掉 TUI 再打开,用 /continue 只会提示重启。现在它会自动加载持久化的 plan.json,重新贴出计划(已完成的步骤打勾)、只跑剩余步骤续上,不用重头再来"]] },
  ...
  en: [
        { ver: "1.0.34", date: "2026-07-06", current: true, title: "umadev init understands your project like Claude Code /init - spec-phase misjudgement fix", changes: [["Fixed", "No more false build claimed done but no source at the spec phase: when the base wrote a PRD/architecture/UIUX/SRS doc during a docs/spec turn, ANY workspace write used to be treated as a code build, running the source-present CODE floor which then falsely failed the code-free docs turn every turn. Now a docs write (output/, .md, .umadev/) does not flip to a build; only a real CODE write does, a later code write still flips it, and an empty/unknown path is treated as code so a real hallucination is never masked. The honesty floor itself is untouched"], ["Added", "umadev init now understands your project like Claude Code /init: on (re-)run it detects the stack + build/test/lint commands + indexes the source, and writes a MANAGED Project section into CLAUDE.md; re-running refreshes only that section, preserving the governance preamble and your hand edits. Previously init only wrote a static template and re-running updated nothing - now it truly analyzes the project + refreshes the detailed index/progress"]] },
        { ver: "1.0.33", date: "2026-07-06", current: false, title: "Third-party bases just work, zero config - a base-call concurrency gate roots out the GLM 529 overload", changes: [["Fixed", "Low-concurrency third-party bases (GLM via open.bigmodel.cn) no longer hit a 529 too-many-requests on every turn: UmaDev is a team and can hold several base processes at once (the resident chat session, a background pre-warmed session, one forked session per reviewing critic), whereas a direct base session holds exactly one connection; those extra concurrent connections exceed a third-party gateway tiny concurrency budget and it rejects the overflow with 529. A new global base-call concurrency gate (base_gate) makes every real base turn acquire one permit; default budget 1 = the same gateway footprint as a direct session, so connect = it just works for ANY base, zero config"], ["Improved", "Speed opt-in for strong endpoints: users on a high-concurrency official base can set UMADEV_BASE_CONCURRENCY=4 to restore parallel critic review + pre-warm; an invisible knob for advanced users, never needed by anyone else"], ["Improved", "Gate internals: permits are per-turn (never held while acquiring another) so it cannot deadlock; pre-warm uses try_acquire and skips rather than blocking your turn; chat firmware was already the Light tier (identity only), so per-call weight is unchanged"]] },
        { ver: "1.0.32", date: "2026-07-06", current: false, title: "Research-backed dev-team interaction hardening · typed hand-off contracts wired end-to-end · gate-display consistency", changes: [["Added", "Seat Cards: every seat (PM/architect/designer/frontend/backend/QA/security/DevOps) carries a typed self-describing capability card declaring what it owns, which contract inputs it reads, and what it produces — the roster is self-describing and every hand-off becomes an explicit, checkable contract"], ["Added", "Per-hop hand-off check (both directions): before a seat runs, its declared inputs/outputs are checked against the blackboard — a missing declared input (misalignment) or an un-materialized owned artifact (a spec gap) surfaces in the verdict at the hop, not as a downstream mystery; live in the review flow"], ["Added", "Two-layer artifact materialization: the decisions in the prose docs (data model / design tokens / acceptance criteria) are parsed into typed contracts emitted to .umadev/contracts/, alongside the API contract, wired into the docs stage"], ["Added", "Structured verdict provenance: a verdict can carry its originating seat + related artifact + diagnosed note, populated deterministically (never trusted from the base) for diagnosed rework + a reconstructable audit trail; serde-backward-compatible"], ["Added", "Artifact versioning → DAG staleness invalidation: blackboard docs carry a content version; edit a PRD/architecture/UIUX doc between sessions and on resume the consuming plan steps + their downstream re-open to re-derive against the changed upstream, instead of trusting a silently-poisoned result"], ["Added", "Private blackboard scratch lane: two seats resolving a conflict work in a private .umadev/scratch/ lane without polluting the public output/ board, GC'd on run end"], ["Fixed", "Gate-display consistency: the preflight gate line is generated from the active trust mode (auto/guarded/plan), so it no longer reads per-gate approval at the bottom while the body says auto-approved; removed the duplicated full-build line on the intent card"]] },
