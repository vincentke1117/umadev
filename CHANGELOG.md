...
## [1.0.16] — 一个模拟真实开发团队来工作的 Coding Agent · 记忆不丢 · 写文档不再烧全流程 · 开发团队架构做实(Wave A/B/C)· 深读 Claude Code 源码落地一大批

系统深读 Claude Code 完整源码 + 互联网商业级工程研究并落地一大批强化;根治用户实测的**记忆丢失**与**文档烧 token**;重定位为**一个模拟真实开发团队来工作的 Coding Agent**(八角色各干各的活、协调者调度)。

### 新增 — 开发团队架构做实

- **Wave A 智能席位建造**:完整构建按 `RoutePlan.depth.is_deliberate()` 自动逐角色真建造(每角色真建自己那摊),小任务仍单轮便宜 —— router 自动判,不让用户选。
- **Wave B 角色真产物**:design-tokens 成一等产物 + `DesignTokensPresent` 验收;契约优先 DAG(前后端依赖架构师契约);QA 先写测试(测试作者≠代码作者去偏)。
- **Wave C 团队可见**:实时**花名册面板**(席位+状态)+ 交接时间线 + 团队**章程**(`/constitution`)+ `/team`;反剧场铁律(没真产物的席位不显示)。

### 新增 — 记忆 · 深读 Claude Code 源码 + 商业级研究落地

- **持久项目事实记忆**:`.umadev/memory/facts.jsonl` 每轮注入固件 —— 已记事实(JDK17 路径/构建命令)永不重查。
- **token 预算自动压缩**:结构化摘要替掉有损 FIFO + `/compact`,长会话早期决策不再被悄悄丢;全转录落盘保全、`/resume` 无损。
- **测试完整性守卫(UD-QA-001)** 反测试造假(删测试/弱化断言/改框架配置骗绿)· **信任档 mode-aware + 自学习** · **TUI 性能**(渲染缓存 + 事件合并,治长会话发沉/流式卡顿)· **可恢复编辑**(kill-ring + 撤销/重做)· 重试可见 · 任务持久化 · 配置迁移器 · 字素簇光标 · 大粘贴折 chip · **首过验收率** + **爆炸半径验证排序**(自进化)· 完成响铃 · `Ctrl+F` 转录搜索 · 上下文/花费仪表 · 双击 Esc 回退 · `!` 命令 · 快捷键速查。
- **知识库四波 +32 商业级标准** + 隐私洁净化 · **MCP 扩到 6 工具**(plan/contract/lessons/governance)· **PostToolUse 审计钩子** · **自定义团队角色**(`.umadev/agents/*.md`)。

### 修复

- **写文档不再烧全流程**:brain-first 文档定规模 —— 大脑判"写文档 vs 做产品",文档轻触至多 1 席 PM、真产品构建不变;源码存在性地板文档感知(纯文档不再被逼"去写代码")。
- 底座空闲 **300s 误杀** → 活性判断(在跑工具且底座活着就一直等)· **中止状态同步** · 路由失败**"继续"不再重头查询**(底座活着留住会话)· 工作时**屏幕闪烁** · **中文吞字**(宽 emoji 标记错位)· **stderr ANSI 乱码** · **滚轮拖选复制** · **多目录串台隔离** · **API 报错不再静默** · codex `/sandbox` 可配。

### 变更

- **定位重写**:全仓库 + 官网统一为"一个模拟真实开发团队来工作的 Coding Agent",团队当主角、协调者调度;版本徽章改为滚动 `1.0.x`。
